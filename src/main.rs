...
use zkbob_cloud::{Engine, config::Config, errors::CloudError, version, cloud::{cloud::ZkBobCloud, signup, short_info, generate_shielded_address, history}};
...
async fn main() -> std::io::Result<()> {
    ...
    let cloud = Data::new(ZkBobCloud::new(config.clone(), pool, pool_id).expect("failed to init cloud"));
